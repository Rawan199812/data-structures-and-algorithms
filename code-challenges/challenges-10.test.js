...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArray = input.map(value => value.filter(value2 => typeof (value2) === 'number' && !(value2 % 5)).map(value3 => Math.pow(2, value3)));
  return newArray;
};
...
let findShortest = (data) => {
  // Solution code here...
  let newArray = data.sort((elemnt1,elemnt2) => elemnt1.height - elemnt2.height ).map(element=>element.name)
  return newArray[0];
};
